public class FLUTE{
Flute f => JCRev rev => Gain gain => dac;
0.1=>rev.mix;
0.2=> gain.gain;


  fun void playNote(int midiNote,dur duration){
    Std.mtof(midiNote)=>f.freq;
    1 => f.noteOn;
    duration=>now;
    0 => f.noteOff;
  }
...
Moog moog => JCRev rev => Gain gain=> dac;
 0.2 => rev.mix;
 0.8 => gain.gain;
 //20=> b3.lfoSpeed;
 //0.5 => b3.lfoDepth;
...
MOOG m;
2::second => dur bar;
//spork ~m.loopChordProgression(Edorian.myScale,[23,25,27,29],bar);
//spork ~ organ.loopIntervals(Edorian.myScale,28,7,5,(bar/7),"up");
spork ~m.loopBassLineProgressionThirds(Edorian.myScale,[28,30,31,33],20,(bar/16),bar);
*/
