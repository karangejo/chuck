// Modal Peak Identification/Extraction Lab
...
while (in.pos() < in.samples())	{   // Average (add) up all frame magnitudes
...
// OK, one last pass to generate the residue
0 => in.pos;  // reset our soundfile to play again

"Residue.wav" => residue.wavFilename;

while (in.pos() < in.samples())	{
    (SIZE/16) :: samp => now;		// Hop size
    fft.upchuck() @=> blob;		    // complex data
    for (0 => int i; i < SIZE/2; i++)	{
        blob.cval(i) @=> Z[i];	//     complex spectrum
    }
    for (0 => pk; pk < NUM_PEAKS; pk++)  {
        squelchComplex(Z,peakloc[pk]);
    }
    ifft.transform(Z);
}
...
<<< "Also go edit the residue file.  Normalize, trim start, shorten...","\n" >>>;
