...
Noise n;
ADSR env[3]; BiQuad b[3]; Pan2 p[3];
NRev r[2];
r[0] => dac.left;  r[1] => dac.right;   // one left, one right reverb
...
while (PROB > 0.001)	{
    ms => now;
    if (Math.random2f(0.0,1.0) < PROB) clap();
    1.002 /=> PROB; // decrease number of clappers
}
<<< now/second >>>;

fun void clap()  {  // make a (random) clap
    (which++ % 3) => which; // grab the least recent dude
    Math.random2f(900,2200) => b[which].pfreq; // pick a frequency
    Math.random2f(0.95,0.98) => b[which].prad;
